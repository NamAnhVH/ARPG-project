...
func _process(delta):
	if has_node("./Slimes"):
		for slime: Slime in get_node("./Slimes").get_children():
	...
	if has_node("./Gremlins"):
		for gremlin: Gremlin in get_node("./Gremlins").get_children():
	...
	if has_node("./Mushrooms"):
		for mushroom: Mushroom in get_node("./Mushrooms").get_children():
